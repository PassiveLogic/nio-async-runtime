...
class EventLoopFutureTest {
  ...
  @Test
  func testFoldWithFailureAndEmptyFutureList() async throws {
    struct E: Error {}
    let eventLoop = makeEventLoop()
  ...
  @Test
  func testFoldWithFailureAndAllSuccesses() async throws {
    struct E: Error {}
    let eventLoop = makeEventLoop()
    let secondEventLoop = makeEventLoop()
    let f0: EventLoopFuture<Int> = eventLoop.makeFailedFuture(E())
  ...
  @Test
  func testFoldWithFailureAndAllUnfulfilled() async throws {
    struct E: Error {}
    let eventLoop = makeEventLoop()
    let secondEventLoop = makeEventLoop()
    let f0: EventLoopFuture<Int> = eventLoop.makeFailedFuture(E())
  ...
  @Test
  func testFoldWithFailureAndAllFailures() async throws {
    struct E: Error {}
    let eventLoop = makeEventLoop()
    let secondEventLoop = makeEventLoop()
    let f0: EventLoopFuture<Int> = eventLoop.makeFailedFuture(E())
  ...
  @Test
  func testAndAllWithAllFailures() async throws {
    struct E: Error {}
    let eventLoop = makeEventLoop()
    let promises: [EventLoopPromise<Void>] = (0..<100).map { (_: Int) in eventLoop.makePromise() }
    let futures = promises.map { $0.futureResult }
  ...
  @Test
  func testAndAllWithOneFailure() async throws {
    struct E: Error {}
    let eventLoop = makeEventLoop()
  ...
  @Test
  func testReduceWithAllFailures() async throws {
    struct E: Error {}
    let eventLoop = makeEventLoop()
  ...
  @Test
  func testReduceWithOneFailure() async throws {
    struct E: Error {}
    let eventLoop = makeEventLoop()
  ...
  @Test
  func testReduceWhichDoesFailFast() async throws {
    struct E: Error {}
    let eventLoop = makeEventLoop()
    var promises: [EventLoopPromise<Int>] = (0..<100).map { (_: Int) in eventLoop.makePromise() }
  ...
  @Test
  func testReduceIntoWithAllFailure() async throws {
    struct E: Error {}
    let eventLoop = makeEventLoop()
    let futures: [EventLoopFuture<Int>] = [1, 2, 2, 3, 3, 3].map { (id: Int) in
  ...
  func testOrderOfFutureCompletion() throws {
    ...
    let completion = eventLoop.submit {
      ...
      let p: EventLoopPromise<Void> = EventLoopPromise(
        eventLoop: eventLoop, file: #filePath, line: #line)
  ...
  @Test
  func testEventLoopHoppingInThen() async throws {
    let n = 20
    ...
    let result = try await prev.get()
    #expect(n - 1 == result)
    #expect(throws: Never.self) { try elg.syncShutdownGracefully() }
  }

  @Test
  func testEventLoopHoppingInThenWithFailures() async throws {
  ...
  @Test
  func testEventLoopHoppingAndAllWithFailures() async throws {
  ...
  @Test
  func testWhenAllSucceedFailsImmediately() async {
    let group = MultiThreadedEventLoopGroup(numberOfThreads: 2)
    defer {
      #expect(throws: Never.self) { try group.syncShutdownGracefully() }
    }

    func doTest(promise: EventLoopPromise<[Int]>?) async {
      let promises = [
      ...
      await #expect(throws: EventLoopFutureTestError.self) {
        try await futureResult.get()
      }
    }

    await doTest(promise: nil)
    await doTest(promise: group.next().makePromise())
  }
  ...
  @Test
  func testWhenAllCompleteResults() async throws {
    let group = MultiThreadedEventLoopGroup(numberOfThreads: 2)
    defer {
      #expect(throws: Never.self) { try group.syncShutdownGracefully() }
    }

    func doTest(promise: EventLoopPromise<[Result<Int, Error>]>?) async throws {
      let futures: [EventLoopFuture<Int>] = [
    ...
    await #expect(throws: Never.self) { try await doTest(promise: nil) }
    await #expect(throws: Never.self) { try await doTest(promise: group.next().makePromise()) }
  }
  ...
  @Test
  func testAlwaysWithFailingPromise() async throws {
    let group = makeEventLoop()
    let loop = group.next()
    ...
    await #expect(throws: DatabaseError.self) {
      try await db.runQuery().always { result in
        assertFailure(result)
        db.close()
      }.map { $0.map { $0.uppercased() } }.get()
    }
    #expect(db.closed)
  }
  ...
  @Test
  func testPromiseCompletedWithFailedFuture() async throws {
    let group = makeEventLoop()
    let loop = group.next()
  ...
  @Test
  func testPromiseCompletedWithFailedResult() async throws {
    let group = makeEventLoop()
    let loop = group.next()
  ...
  @Test
  func testAndAllCompleteWithPreSucceededFutures() async {
    let eventLoop = makeEventLoop()
    let succeeded = eventLoop.makeSucceededFuture(())

    for i in 0..<10 {
      await #expect(throws: Never.self) {
        try await EventLoopFuture<Void>.andAllComplete(
  ...
  @Test
  func testAndAllCompleteWithPreFailedFutures() async {
    struct Dummy: Error {}
    let eventLoop = makeEventLoop()
  ...
  func testRepeatedTaskOffEventLoopGroupFuture() throws {
    ...
    _ = elg1.next().scheduleRepeatedAsyncTask(initialDelay: .nanoseconds(0), delay: .nanoseconds(0))
    { task in
      struct Dummy: Error {}
  ...
  @Test
  func testEventLoopFutureOrThrows() async {
    let eventLoop = makeEventLoop()
    let promise = eventLoop.makePromise(of: Int?.self)
    let result: Result<Int?, Error> = .success(nil)
    promise.completeWith(result)

    await #expect(throws: EventLoopFutureTestError.example) {
      try await promise.futureResult.unwrap(orError: EventLoopFutureTestError.example).get()
    }
  }
  ...
  @Test
  func testFlatBlockingMapOnto() async {
    ...
    p.futureResult.map {
      ...
      let nonBlockingRanResult = blockingRan.load(ordering: .sequentiallyConsistent)
  ...
  @Test
  func testFlatMapWithEL() async throws {
    let el = makeEventLoop()

    let result = try await el.makeSucceededFuture(1).flatMapWithEventLoop { one, el2 in
      #expect(el === el2)
  ...
  @Test
  func testFlatMapErrorWithEL() async throws {
    let el = makeEventLoop()
    struct E: Error {}

    let result = try await el.makeFailedFuture(E()).flatMapErrorWithEventLoop { error, el2 in
      #expect(error is E)
  ...
  @Test
  func testFoldWithEL() async throws {
    let el = makeEventLoop()
  ...
  func testPreconditionFailure() async {
    ...
    await #expect(throws: EventLoopFutureTestError.example) {
      try await preconditionedFuture.get()
    }
  }
